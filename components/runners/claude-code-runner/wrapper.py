...
class ClaudeCodeAdapter:
    ...
    def _is_valid_git_branch_name(branch: str) -> bool:
        ...
        Defense-in-depth validation matching backend's isValidGitBranchName().
        Even though backend validates, runner must also validate to prevent:
        - Bypass if backend validation is incomplete
        - Future code changes that bypass backend
        - Direct runner invocation in testing/debugging
        ...
        # CRITICAL: Reject shell metacharacters that could lead to command injection
        # These characters could break out of git commands
        shell_metachars = {';', '&', '|', '$', '`', '\\', '\n', '\r', '\t', '<', '>',
                          '(', ')', '{', '}', "'", '"', ' '}
        if any(char in branch for char in shell_metachars):
            return False

        # Reject git control characters and patterns
        git_control_patterns = ["..", "~", "^", ":", "?", "*", "[", "@{"]
        if any(pattern in branch for pattern in git_control_patterns):
            return False

        # Cannot start or end with dot or slash
        if branch.startswith('.') or branch.endswith('.') or \
           branch.startswith('/') or branch.endswith('/'):
            return False
    ...
    async def _clone_repository_with_error_handling(
        self,
        url: str,
        target_dir: Path,
        token: str = None,
        ...
        """
        Clone a repository with graceful error handling.

        The working_branch parameter is now generated by the backend API and passed in the spec.
        This method simply clones the repository and checks out/creates the specified branch.

        Workflow:
        1. Clone repository (default branch)
        2. If working_branch specified:
           - Try to checkout if exists remotely
           - Create from default branch if doesn't exist
        ...
        try:
            ...
            logging.info(f"Cloning {repo_name} from {url}")

            await self._run_cmd(
                ["git", "clone", clone_url, str(target_dir)],
                cwd=str(workspace or target_dir.parent)
            )

            # Get the default branch name
            default_branch_result = await self._run_cmd(
                ["git", "branch", "--show-current"],
                cwd=str(target_dir)
            )
            default_branch = default_branch_result.get('stdout', '').strip() or 'main'
            logging.info(f"Repository default branch: {default_branch}")

            # If working branch specified, checkout or create it
            if working_branch:
                # Validate branch name to prevent command injection (defense-in-depth)
                if not self._is_valid_git_branch_name(working_branch):
                    error_msg = f"Invalid or potentially unsafe branch name: {working_branch}"
                    logging.error(f"Branch name validation failed for {repo_name}: {error_msg}")
                    await self._send_log(f"✗ {error_msg}")
                    raise RuntimeError(error_msg)

                # Try to checkout the working branch (may exist remotely or locally)
                try:
                    # First try remote branch
                    await self._run_cmd(["git", "checkout", working_branch], cwd=str(target_dir))
                    await self._send_log(f"✓ Checked out branch '{working_branch}'")
                    logging.info(f"Checked out existing branch '{working_branch}' in {repo_name}")
                except RuntimeError:
                    # Branch doesn't exist - create it from default branch
                    await self._run_cmd(["git", "checkout", "-b", working_branch], cwd=str(target_dir))
                    await self._send_log(f"✓ Created branch '{working_branch}' from '{default_branch}'")
                    logging.info(f"Created new branch '{working_branch}' from '{default_branch}' in {repo_name}")
            else:
                # No branch specified - stay on default branch
                await self._send_log(f"✓ Using default branch '{default_branch}'")
                logging.info(f"Using default branch '{default_branch}' in {repo_name}")
            ...
            logging.info(f"Successfully cloned and configured {repo_name}")
            return True
            ...
            if "authentication" in error_msg or "permission denied" in error_msg or "could not read" in error_msg:
                await self._send_log(f"⚠️ Authentication failed for {repo_name} - continuing without this repository")
                logging.warning(f"Authentication error cloning {repo_name}: {clone_err}")
                return False

            # Other git errors
            else:
                await self._send_log(f"⚠️ Failed to clone {repo_name}: {clone_err} - continuing without this repository")
                logging.warning(f"Clone error for {repo_name}: {clone_err}")
                return False
        ...
        if repos_cfg:
            ...
            try:
                for r in repos_cfg:
                    ...
                    # Get branch from spec (generated by backend API)
                    # The backend now handles branch name generation including protected branch logic
                    ...
                    if not repo_exists:
                        # Clone fresh copy with graceful error handling
                        clone_success = await self._clone_repository_with_error_handling(
                            url=url,
                            target_dir=repo_dir,
                            token=token,
                            repo_name=name,
                            ...
                            allow_protected=False  # Not needed - backend handles this
                        )

                        if not clone_success:
                            logging.warning(f"Skipping {name} due to clone failure")
                            continue

                        # Setup sync remote if configured
                        if sync_config.get('url'):
                            sync_url = sync_config['url'].strip()
                            sync_branch = (sync_config.get('branch') or '').strip() or 'main'

                            # Validate sync branch name to prevent command injection
                            if not self._is_valid_git_branch_name(sync_branch):
                                error_msg = f"Invalid or potentially unsafe sync branch name: {sync_branch}"
                                logging.error(f"Sync branch validation failed for {name}: {error_msg}")
                                await self._send_log(f"✗ {error_msg}")
                                raise RuntimeError(error_msg)

                            try:
                                sync_url_with_token = self._url_with_token(sync_url, token) if token else sync_url
                                await self._run_cmd(["git", "remote", "add", "upstream", sync_url_with_token], cwd=str(repo_dir))
                                await self._send_log(f"✓ Added upstream remote for {name}: {sync_url}")

                                # Fetch and rebase onto the sync branch
                                await self._run_cmd(["git", "fetch", "upstream", sync_branch], cwd=str(repo_dir))
                                await self._run_cmd(["git", "rebase", f"upstream/{sync_branch}"], cwd=str(repo_dir))
                                current_branch_result = await self._run_cmd(["git", "branch", "--show-current"], cwd=str(repo_dir))
                                current_branch = current_branch_result.get('stdout', '').strip()
                                await self._send_log(f"✓ Rebased {current_branch} onto upstream/{sync_branch}")
                                logging.info(f"Rebased {current_branch} onto upstream/{sync_branch} in {name}")
                            except RuntimeError as e:
                                logging.warning(f"Failed to setup upstream remote for {name}: {e}")
        ...
        try:
            if not workspace_has_git:
                ...
                clone_success = await self._clone_repository_with_error_handling(
                    ...
                    allow_protected=False  # For legacy single-repo, use default protection
                )

                if not clone_success:
                    logging.warning("Failed to clone input repository, session will continue without repository context")
                    return
            elif reusing_workspace:
                # Reusing workspace - preserve local changes from previous session
