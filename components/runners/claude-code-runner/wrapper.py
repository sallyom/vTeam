...
class ClaudeCodeAdapter:
    ...
    def __init__(self):
        ...
        self._first_run = (
            True  # Track if this is the first SDK run or a mid-session restart
        )
    ...
    def _sanitize_user_context(user_id: str, user_name: str) -> tuple[str, str]:
        ...
        if user_id:
            ...
            if len(user_id) > 255:
                logging.warning(
                    f"User ID exceeds max length (255), truncating: {len(user_id)} chars"
                )
                user_id = user_id[:255]
            ...
            if sanitized_id != user_id:
                logging.warning(
                    f"User ID contained invalid characters, sanitized from {len(user_id)} to {len(sanitized_id)} chars"
                )
        ...
        if user_name:
            ...
            if len(user_name) > 255:
                logging.warning(
                    f"User name exceeds max length (255), truncating: {len(user_name)} chars"
                )
            ...
            if sanitized_name != user_name:
                logging.warning(
                    f"User name contained control characters, sanitized from {len(user_name)} to {len(sanitized_name)} chars"
                )
    ...
    async def run(self):
        ...
        try:
            ...
            if not prompt:
                prompt = self.context.get_metadata(
            ...
            try:
                auto_push = str(
                    self.context.get_env("AUTO_PUSH_ON_COMPLETE", "false")
            ...
            try:
                if isinstance(result, dict) and result.get("success"):
                    logging.info(
                        f"Updating CR status to Completed (result.success={result.get('success')})"
                    )
                    ...
                    await self._update_cr_status(
                        {
                            ...
                            "subtype": (result.get("result") or {}).get(
                                "subtype", "success"
                            ),
                            "is_error": False,
    ...
    async def _run_claude_agent_sdk(self, prompt: str):
        ...
        try:
            ...
            if use_vertex:
                ...
                logging.info("Vertex AI environment configured:")
                logging.info(
                    f"  CLAUDE_CODE_USE_VERTEX: {os.environ.get('CLAUDE_CODE_USE_VERTEX')}"
                )
                logging.info(
            ...
            if active_workflow_url:
                ...
                try:
                    ...
                    logging.warning(
                        f"Failed to derive workflow name: {e}, using default"
                    )
                    cwd_path = str(
                        Path(self.context.workspace_path) / "workflows" / "default"
                    )
            ...
            logging.info(
                f"Applied workspace context system prompt (length: {len(workspace_prompt)} chars)"
            )
            ...
            if is_continuation and parent_session_id:
                try:
                    ...
                    if sdk_resume_id:
                        ...
                        logging.info(
                            f"Enabled SDK session resumption: resume={sdk_resume_id[:8]}, fork=False"
                        )
                        await self._send_log(
                            f"🔄 Resuming SDK session {sdk_resume_id[:8]}"
                        )
                    else:
                        logging.warning(
                            f"Parent session {parent_session_id} has no stored SDK session ID, starting fresh"
                        )
                        await self._send_log(
                            "⚠️ No SDK session ID found, starting fresh"
                        )
                except Exception as e:
            ...
            interactive = str(
                self.context.get_env("INTERACTIVE", "false")
            ).strip().lower() in ("1", "true", "yes")
                ...
                async for message in client_obj.receive_response():
                    ...
                    if isinstance(message, SystemMessage):
                        if message.subtype == "init" and message.data.get("session_id"):
                            ...
                            try:
                                await self._update_cr_annotation(
                                    "ambient-code.io/sdk-session-id", sdk_session_id
                                )
                            except Exception as e:
                                logging.warning(
                                    f"Failed to store SDK session ID in CR annotations: {e}"
                                )

                    if isinstance(message, (AssistantMessage, UserMessage)):
                        # Track Claude generation in Langfuse
                        if isinstance(message, AssistantMessage):
                            model_name = getattr(options, "model", None) or os.getenv(
                                "LLM_MODEL", "claude-3-5-sonnet-20241022"
                            )
                            obs.track_generation(message, model_name, self._turn_count)
                        ...
                        # Track session-level cost and usage
                        obs.track_session_totals(result_payload)
            ...
            async with ClaudeSDKClient(options=options) as client:
                if is_continuation and parent_session_id:
                    ...
                    logging.info(
                        f"SDK is handling session resumption for {parent_session_id}"
                    )
                ...
                if not is_continuation:
                    if ambient_config.get("startupPrompt") and not self._first_run:
                        ...
                        logging.info(
                            f"Sent workflow startupPrompt ({len(startup_msg)} chars)"
                        )
                        ...
                        logging.info(
                            "No initial prompt - Claude will greet based on system prompt"
                        )
                else:
                ...
                if interactive:
                    ...
                    while True:
                        ...
                        if mtype in ("user_message", "user_message"):
                            ...
                            await self._send_log(
                                {"level": "system", "message": "interactive.ended"}
                            )
                            break
                            ...
                            if git_url:
                                await self._handle_workflow_selection(
                                    git_url, branch, path
                                )
                                ...
                                await self._send_log(
                                    "⚠️ Workflow change request missing gitUrl"
                                )
                            ...
                            try:
                                ...
                                await self._send_log(
                                    {"level": "info", "message": "interrupt.sent"}
                                )
                            except Exception as e:
                                await self._send_log(
                                    {
                            ...
                            await self._send_log(
                                {
                                    "level": "debug",
                                    "message": f"ignored.message: {mtype_raw}",
                                }
                            )
    ...
    async def _prepare_workspace(self):
        ...
        logging.info(
            f"Workspace preparation: parent_session_id={parent_session_id[:8] if parent_session_id else 'None'}, reusing={reusing_workspace}"
        )
        if reusing_workspace:
            await self._send_log(
                f"♻️ Reusing workspace from session {parent_session_id[:8]}"
            )
            ...
            try:
                for r in repos_cfg:
                    ...
                    if not repo_exists:
                        ...
                        logging.info(
                            f"Repo {name} exists and reusing workspace - preserving all local changes"
                        )
                        ...
                        await self._run_cmd(
                            [
                                ...
                                "set-url",
                                "origin",
                                self._url_with_token(url, token) if token else url,
                            ],
                            cwd=str(repo_dir),
                            ignore_errors=True,
                        )
                        ...
                        logging.info(
                            f"Repo {name} exists but not reusing - resetting to clean state"
                        )
                        await self._run_cmd(
                            [
                                "git",
                        ...
                        await self._run_cmd(
                            ["git", "fetch", "origin", branch], cwd=str(repo_dir)
                        )
                        await self._run_cmd(
                    ...
                    user_name = (
                        os.getenv("GIT_USER_NAME", "").strip() or "Ambient Code Bot"
                    )
                    user_email = (
                        os.getenv("GIT_USER_EMAIL", "").strip()
                        or "bot@ambient-code.local"
                    )
                    await self._run_cmd(
                        ["git", "config", "user.name", user_name], cwd=str(repo_dir)
                    )
                    await self._run_cmd(
                        ["git", "config", "user.email", user_email], cwd=str(repo_dir)
                    )
                    ...
                    if out_url_raw:
                        out_url = (
                            self._url_with_token(out_url_raw, token)
                            if token
                            else out_url_raw
                        )
                        await self._run_cmd(
        ...
        logging.info(
            f"Single-repo setup: workspace_has_git={workspace_has_git}, reusing={reusing_workspace}"
        )
        ...
        try:
            if not workspace_has_git:
                ...
                clone_url = (
                    self._url_with_token(input_repo, token) if token else input_repo
                )
                await self._run_cmd(
                    [
                        "git",
                ...
                logging.info(
                    "Workspace exists and reusing - preserving all local changes"
                )
                await self._run_cmd(
                    [
                        "git",
                        ...
                        "origin",
                        (
                            self._url_with_token(input_repo, token)
                            if token
                            else input_repo
                        ),
                    ],
                ...
                logging.info(
                    "Workspace exists but not reusing - resetting to clean state"
                )
                await self._run_cmd(
                    [
                        "git",
                ...
                await self._run_cmd(
                    ["git", "fetch", "origin", input_branch], cwd=str(workspace)
                )
                await self._run_cmd(
                    ["git", "checkout", input_branch], cwd=str(workspace)
                )
                await self._run_cmd(
                    ["git", "reset", "--hard", f"origin/{input_branch}"],
                    cwd=str(workspace),
                )
            ...
            user_email = (
                os.getenv("GIT_USER_EMAIL", "").strip() or "bot@ambient-code.local"
            )
            await self._run_cmd(
                ["git", "config", "user.name", user_name], cwd=str(workspace)
            )
            await self._run_cmd(
                ["git", "config", "user.email", user_email], cwd=str(workspace)
            )
            logging.info(f"Git identity configured: {user_name} <{user_email}>")
            ...
            if output_repo:
                ...
                out_url = (
                    self._url_with_token(output_repo, token) if token else output_repo
                )
                await self._run_cmd(
                    ["git", "remote", "remove", "output"],
                    cwd=str(workspace),
                ...
                await self._run_cmd(
                    ["git", "remote", "add", "output", out_url], cwd=str(workspace)
                )
    ...
    async def _validate_prerequisites(self):
        ...
        prerequisites = {
            "/speckit.plan": (
                "spec.md",
                "Specification file (spec.md) not found. Please run /speckit.specify first to generate the specification.",
            ),
            "/speckit.tasks": (
                "plan.md",
                "Planning file (plan.md) not found. Please run /speckit.plan first to generate the implementation plan.",
            ),
            "/speckit.implement": (
                "tasks.md",
                "Tasks file (tasks.md) not found. Please run /speckit.tasks first to generate the task breakdown.",
            ),
        }
    ...
    async def _initialize_workflow_if_set(self):
        ...
        try:
            ...
            if not derived_name:
                logging.warning(
                    "Could not derive workflow name from URL, skipping initialization"
                )
                return
            ...
            if workflow_dir.exists():
                logging.info(
                    f"Workflow {derived_name} already exists, skipping initialization"
                )
                return

            logging.info(
                f"Initializing workflow {derived_name} from CR spec on startup"
            )
            # Clone the workflow but don't request restart (we haven't started yet)
            await self._clone_workflow_repository(
                active_workflow_url,
    ...
    async def _clone_workflow_repository(
        ...
        logging.info("Successfully cloned workflow to temp directory")
        ...
        if path and path.strip():
            ...
            temp_clone_dir.rename(workflow_dir)
            logging.info("Using entire repository as workflow")

        await self._send_log(f"✅ Workflow {workflow_name} ready")
        logging.info(f"Workflow {workflow_name} setup complete at {workflow_dir}")

    async def _handle_workflow_selection(
        self, git_url: str, branch: str = "main", path: str = ""
    ):
    ...
    async def _handle_repo_added(self, payload):
        ...
        repos_cfg = self._get_repos_config()
        repos_cfg.append(
            {"name": repo_name, "input": {"url": repo_url, "branch": repo_branch}}
        )
    ...
    async def _push_results_if_any(self):
        ...
        if token:
            ...
            logging.warning(
                "No GitHub token available - push may fail for private repos"
            )
        ...
        if repos_cfg:
            ...
            try:
                for r in repos_cfg:
                    ...
                    if not out_url_raw:
                        logging.warning(
                            f"No output URL configured for {name}, skipping push"
                        )
                        continue
                    ...
                    out_branch = (
                        out.get("branch") or ""
                    ).strip() or f"sessions/{self.context.session_id}"

                    await self._send_log(f"Pushing changes for {name}...")
                    logging.info(
                        f"Configuring output remote with authentication for {name}"
                    )
                    ...
                    await self._run_cmd(
                        ["git", "remote", "add", "output", out_url], cwd=str(repo_dir)
                    )

                    logging.info(f"Checking out branch {out_branch} for {name}")
                    await self._run_cmd(
                        ["git", "checkout", "-B", out_branch], cwd=str(repo_dir)
                    )
                    ...
                    try:
                        await self._run_cmd(
                            [
                                ...
                                "commit",
                                "-m",
                                f"Session {self.context.session_id}: update",
                            ],
                            cwd=str(repo_dir),
                        )
                    ...
                    remotes_output = await self._run_cmd(
                        ["git", "remote", "-v"], cwd=str(repo_dir), capture_stdout=True
                    )
                    logging.info(
                        f"Git remotes for {name}:\n{self._redact_secrets(remotes_output)}"
                    )
                    ...
                    await self._run_cmd(
                        ["git", "push", "-u", "output", f"HEAD:{out_branch}"],
                        cwd=str(repo_dir),
                    )
                    ...
                    create_pr_flag = (
                        os.getenv("CREATE_PR", "").strip().lower() == "true"
                    )
                    if (
                        create_pr_flag
                        and in_branch
                        and out_branch
                        and out_branch != in_branch
                        and out_url
                    ):
                        upstream_url = (in_.get("url") or "").strip() or out_url
                        target_branch = (
                            os.getenv("PR_TARGET_BRANCH", "").strip() or in_branch
                        )
                        try:
                            pr_url = await self._create_pull_request(
                                upstream_repo=upstream_url,
                                fork_repo=out_url,
                                head_branch=out_branch,
                            ...
                            if pr_url:
                                await self._send_log(
                                    {
                                        "level": "info",
                                        "message": f"Pull request created for {name}: {pr_url}",
                                    }
                                )
                        except Exception as e:
                            await self._send_log(
                                {
                                    "level": "error",
                                    "message": f"PR creation failed for {name}: {e}",
                                }
                            )
            except Exception as e:
        ...
        if not output_repo_raw:
            logging.info(
                "No OUTPUT_REPO_URL configured, skipping legacy single-repo push"
            )
            return

        # Add token to output URL
        output_repo = (
            self._url_with_token(output_repo_raw, token) if token else output_repo_raw
        )

        output_branch = (
            os.getenv("OUTPUT_BRANCH", "").strip()
            or f"sessions/{self.context.session_id}"
        )
        input_repo = os.getenv("INPUT_REPO_URL", "").strip()
        ...
        try:
            ...
            if not status.strip():
                await self._send_log(
                    {"level": "system", "message": "No changes to push."}
                )
                return
            ...
            await self._run_cmd(
                ["git", "remote", "add", "output", output_repo], cwd=str(workspace)
            )

            logging.info(f"Checking out branch {output_branch}")
            await self._run_cmd(
                ["git", "checkout", "-B", output_branch], cwd=str(workspace)
            )
            ...
            try:
                ...
                if "nothing to commit" in str(e).lower():
                    ...
                    await self._send_log(
                        {"level": "system", "message": "No new changes to commit."}
                    )
                    return
                else:
            ...
            remotes_output = await self._run_cmd(
                ["git", "remote", "-v"], cwd=str(workspace), capture_stdout=True
            )
            ...
            await self._run_cmd(
                ["git", "push", "-u", "output", f"HEAD:{output_branch}"],
                cwd=str(workspace),
            )
            ...
            if (
                ...
                and input_branch
                and output_branch
                and output_branch != input_branch
            ):
                target_branch = (
                    os.getenv("PR_TARGET_BRANCH", "").strip() or input_branch
                )
                try:
                    pr_url = await self._create_pull_request(
                        upstream_repo=input_repo or output_repo,
                        fork_repo=output_repo,
                        head_branch=output_branch,
                        base_branch=target_branch,
                    )
                    if pr_url:
                        await self._send_log(
                    ...
                    await self._send_log(
                        {"level": "error", "message": f"PR creation failed: {e}"}
                    )
        except Exception as e:
            logging.error(f"Failed to push results: {e}")
            await self._send_log(f"Push failed: {e}")
    ...
    async def _create_pull_request(
        self, upstream_repo: str, fork_repo: str, head_branch: str, base_branch: str
    ) -> str | None:
        ...
        token = (
            os.getenv("GITHUB_TOKEN") or await self._fetch_github_token() or ""
        ).strip()
        if not token:
        ...
        def _do_req():
            try:
                ...
                raise RuntimeError(
                    f"GitHub PR create failed: HTTP {he.code}: {err_body}"
                )
            except Exception as e:
    ...
    async def _update_cr_annotation(self, key: str, value: str):
        ...
        try:
            ...
            patch = _json.dumps({"metadata": {"annotations": {key: value}}}).encode(
                "utf-8"
            )
    ...
    async def _update_cr_status(self, fields: dict, blocking: bool = False):
        ...
        def _do():
            try:
                with _urllib_request.urlopen(req, timeout=10) as resp:
                    _ = resp.read()
                logging.info(
                    f"CR status update successful to {fields.get('phase', 'unknown')}"
                )
                return True
            except _urllib_error.HTTPError as he:
                logging.error(
                    f"CR status HTTPError: {he.code} - {he.read().decode('utf-8', errors='replace')}"
                )
                return False
            except Exception as e:
        ...
        if blocking:
            ...
            logging.info(
                f"BLOCKING CR status update to {fields.get('phase', 'unknown')}"
            )
            success = _do()
    ...
    async def _wait_for_ws_connection(self, timeout_seconds: int = 10):
        ...
        while True:
            ...
            if elapsed > timeout_seconds:
                logging.error(
                    f"WebSocket connection not established after {timeout_seconds}s - proceeding anyway"
                )
                return

            try:
                logging.info(
                    f"WebSocket connection established (attempt {attempt + 1})"
                )
                return  # Success!
            except Exception as e:
                attempt += 1
    ...
    async def _get_sdk_session_id(self, session_name: str) -> str:
        ...
        try:
            ...
            if "projects" in path_parts and "agentic-sessions" in path_parts:
                ...
                project = (
                    path_parts[proj_idx + 1] if len(path_parts) > proj_idx + 1 else ""
                )
        ...
        try:
            ...
            if sdk_session_id:
                ...
                if "-" in sdk_session_id and len(sdk_session_id) == 36:
                    logging.info(
                        f"Found SDK session ID in annotations: {sdk_session_id}"
                    )
                ...
                logging.warning(
                    f"Parent session {session_name} has no sdk-session-id annotation"
                )
                return ""
        except Exception as e:
    ...
    async def _send_partial_output(
        self, output_chunk: str, *, stream_id: str, index: int
    ):
    ...
    def _build_workspace_context_prompt(
        ...
        prompt = (
            "You are Claude Code working in a structured development workspace.\n\n"
        )
        ...
        if workflow_name:
            ...
            prompt += (
                "This directory contains workflow logic and automation scripts.\n\n"
            )
        ...
        prompt += (
            "Purpose: Create all output artifacts (documents, specs, reports) here.\n"
        )
        prompt += (
            "This directory persists across workflows and has its own git remote.\n\n"
        )
            ...
            if isinstance(data, list):
                ...
                for it in data:
                    ...
                    if name and isinstance(input_obj, dict) and url:
                        out.append(
                            {"name": name, "input": input_obj, "output": output_obj}
                        )
                return out
        except Exception:
            return []
    ...
    def _filter_mcp_servers(self, servers: dict) -> dict:
        ...
        for name, server_config in servers.items():
            if not isinstance(server_config, dict):
                logging.warning(
                    f"MCP server '{name}' has invalid configuration format, skipping"
                )
                continue
            ...
            if server_type in allowed_types:
                ...
                if url:
                    ...
                    logging.info(
                        f"MCP server '{name}' allowed (type: {server_type}, url: {url})"
                    )
                else:
                    logging.warning(
                        f"MCP server '{name}' rejected: missing 'url' field"
                    )
            else:
                logging.warning(
                    f"MCP server '{name}' rejected: type '{server_type}' not allowed"
                )
    ...
    def _load_mcp_config(self, cwd_path: str) -> dict | None:
        ...
        try:
            ...
            if os.getenv("MCP_CONFIG_SEARCH", "").strip().lower() in (
                "0",
                "false",
                "no",
            ):
            ...
            if explicit_path:
                ...
                if mcp_file.exists() and mcp_file.is_file():
                    ...
                    with open(mcp_file, "r") as f:
                        ...
                        if filtered_servers:
                            logging.info(
                                f"MCP servers loaded: {list(filtered_servers.keys())}"
                            )
                    ...
                    logging.warning(
                        f"MCP_CONFIG_PATH specified but file not found: {explicit_path}"
                    )
            ...
            if mcp_file.exists() and mcp_file.is_file():
                ...
                with open(mcp_file, "r") as f:
                    ...
                    if filtered_servers:
                        logging.info(
                            f"MCP servers loaded from {mcp_file}: {list(filtered_servers.keys())}"
                        )
                        return filtered_servers
                    logging.info("No valid MCP servers found after filtering")
                    return None
            ...
            if self.context and self.context.workspace_path != cwd_path:
                ...
                if workspace_mcp_file.exists() and workspace_mcp_file.is_file():
                    logging.info(
                        f"Found .mcp.json in workspace root: {workspace_mcp_file}"
                    )
                    with open(workspace_mcp_file, "r") as f:
                        ...
                        if filtered_servers:
                            logging.info(
                                f"MCP servers loaded from {workspace_mcp_file}: {list(filtered_servers.keys())}"
                            )
                            return filtered_servers
                        logging.info("No valid MCP servers found after filtering")
                        return None
    ...
    def _load_ambient_config(self, cwd_path: str) -> dict:
        ...
        try:
            ...
            with open(config_path, "r") as f:
                config = _json.load(f)
                logging.info(
                    f"Loaded ambient.json: name={config.get('name')}, artifactsDir={config.get('artifactsDir')}"
                )
                return config
