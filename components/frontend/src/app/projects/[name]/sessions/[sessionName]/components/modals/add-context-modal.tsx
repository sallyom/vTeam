...
export function AddContextModal({
  ...
  // Check if base branch is likely protected
  ...
  const showProtectedWarning = isProtectedBranch(baseBranch) && !featureBranch;
  ...
  const handleSubmit = async () => {
    ...
    // Add feature branch if specified
    if (featureBranch.trim()) {
      config.featureBranch = featureBranch.trim();
    }
        ...
        <div className="space-y-4">
          ...
          <div className="border rounded-lg p-4 space-y-4 bg-muted/30">
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">
                The primary branch to clone from. Defaults to 'main'.
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">
                If specified, all modifications will be made to this branch. Will be created from base branch if it doesn't exist.
              </p>
            </div>
              ...
              <Alert variant="warning" className="border-orange-200 bg-orange-50 dark:bg-orange-950/20">
                ...
                <AlertDescription className="space-y-2">
                  <p className="text-sm text-orange-800 dark:text-orange-200">
                    '<strong>{baseBranch}</strong>' appears to be a protected branch.
                  </p>
                  ...
                  <p className="text-xs text-orange-700 dark:text-orange-300 mt-1">
                    {allowProtectedWork
                      ? "You'll work directly on this branch (use with caution)"
                      : "A working branch will be created automatically to preserve the base branch"}
                  </p>
                </AlertDescription>
              </Alert>
            )}
              ...
              <CollapsibleTrigger asChild>
                <Button variant="ghost" className="w-full justify-between p-0 hover:bg-transparent">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium">Sync with Upstream Repository (optional)</span>
                  </div>
                </Button>
              </CollapsibleTrigger>
              <CollapsibleContent className="space-y-3 mt-3 pl-6 border-l-2 border-muted">
                <div className="space-y-2">
                  <Label htmlFor="sync-url">Upstream Repository URL</Label>
                  <Input
                    id="sync-url"
                  ...
                  />
                  <p className="text-xs text-muted-foreground">
                    Repository to sync from (useful for forks)
                  </p>
                </div>

                <div className="space-y-2">
                  <Label htmlFor="sync-branch">Upstream Branch</Label>
                  <Input
                    id="sync-branch"
                    placeholder="main"
                    value={syncBranch}
                  ...
                  />
                  <p className="text-xs text-muted-foreground">
                    Branch to sync from upstream. Your feature branch will be rebased onto this.
                  </p>
                </div>
                ...
                <Alert className="bg-blue-50 dark:bg-blue-950/20 border-blue-200">
                  ...
                  <AlertDescription className="text-xs text-blue-800 dark:text-blue-200">
                    When configured, the system will run: <code className="bg-blue-100 dark:bg-blue-900 px-1 rounded">git fetch upstream && git rebase upstream/{syncBranch || 'main'}</code> before starting work.
                  </AlertDescription>
                </Alert>
              </CollapsibleContent>
            </Collapsible>
